...
optical character recognition and HTR has traditionally been locked behind some level of technical know-how, requiring 
the setup of models either through pure Python code or via some form of coding notebook. This gatekeeps the use of 
machine learning for HTR from many individuals studying the humanities, who would massively benefit from access to such 
technology if only it was easier to use. LTU HTR attempts to provide this easy-to-use solution for handwritten text 
recognition of historical documents, accessible to everyone.
...
to parse the text from those documents. Once inference has been performed, the resulting text is output for human review 
and available for saving/downloading into text files.
...
control and extensibility. This approach allows for loading different model wrappers and modifying the state of the 
server during runtime, as well as handling message queueing.

### Python Management
Packaged with the application, [uv](https://docs.astral.sh/uv/) handles Python virtual environment management, ensuring
Python and necessary model dependencies are installed and available for use on startup.
...
`src/inference/interface/base_models.py`, and defines two separate abstract model classes for implementing model wrappers:
...
![](images/hisam_cer_module_hierarchy.png)  
...
*Example wrapper hierarchy:*  
![](images/crnn_bentham_module_hierarchy.png) 
